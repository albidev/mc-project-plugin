...
function longSnapshot(base) {
  ...
  for (let index = 0; index < DIFF_LINES; index += 1) diffBody.push(`${index % 3 === 0 ? '+' : index % 3 === 1 ? '-' : ' '}line ${String(index).padStart(4, '0')} ${'x'.repeat(120)}`);
  ...
  snapshot.branches = { ...snapshot.branches, local: [{ name: 'main', current: true, tracking: 'origin/main', remoteAlias: 'origin', repository: snapshot.branches?.repository, relation: 'up-to-date', ahead: 0, behind: 0 }] };
  snapshot.focusDefaults = { kind: 'file', value: 'src/app.ts' };
  const expected = snapshot.capabilities ?? {};
  expected.commits = { ...(expected.commits ?? {}), status: 'ready', stale: false, source: 'local_git', observedAt: snapshot.observedAt, generation: expected.commits?.generation ?? 1, value: commits, staleSince: null };
  expected.branchLogs = { ...(expected.branchLogs ?? {}), status: 'ready', stale: false, source: 'local_git', observedAt: snapshot.observedAt, generation: expected.branchLogs?.generation ?? 1, value: snapshot.branchLogs, staleSince: null };
  expected.branches = { ...(expected.branches ?? {}), status: 'ready', stale: false, source: 'local_git', observedAt: snapshot.observedAt, generation: expected.branches?.generation ?? 1, value: snapshot.branches, staleSince: null };
  snapshot.capabilities = expected;
  return snapshot;
}

const envelope = (data) => ({ ok: true, data, meta: { schemaVersion: 1, requestId: 'issue1-browser', observedAt: '2026-09-14T10:00:00+00:00' } });
...
async function resetAndMeasure(page, testId) {
  await page.evaluate((id) => { const element = document.querySelector(`[data-testid="${id}"]`); if (element) element.scrollTo({ top: 0, left: 0 }); }, testId);
...
async function assertScrollable(page, state, testId, { horizontal }) {
  ...
  if (!before) return;
  check(`${state}/${testId} has vertical overflow`, before.scrollHeight > before.clientHeight, `${before.scrollHeight}/${before.clientHeight}`);
  ...
  metrics.states[`${state}/${testId}/horizontal`] = { scrollWidth: before.scrollWidth, clientWidth: before.clientWidth, needsHorizontal };
  ...
  check(`${state}/${testId} wheel within bounds`, wheel.scrollTop <= Math.max(0, wheel.scrollHeight - wheel.clientHeight), `scrollTop ${wheel.scrollTop}`);
  ...
  else metrics.channelUnavailable = [...(metrics.channelUnavailable ?? []), { state, testId, channel: 'scrollbar-drag', reason: drag.reason }];

  // 3. keyboard
  await resetAndMeasure(page, testId);
  await page.locator(`[data-testid="${testId}"]`).focus();
  const focused = await page.evaluate((id) => document.activeElement === document.querySelector(`[data-testid="${id}"]`), testId);
...
async function assertMobileNonOwner(page, state, testId) {
  ...
  if (!value) return;
  check(`${state}/${testId} is not a nested vertical scroll owner`, value.scrollHeight <= value.clientHeight + 1, `${value.scrollHeight}/${value.clientHeight}`);
...
async function main() {
  ...
  const catalog = [{ project_id: snapshot.project_id, name: snapshot.project?.name ?? 'Demo', enabled: true, remote: 'origin', default_branch: 'main' }];
  ...
  await page.route('**/api/local/mc-project-plugin/projects/**', async (route) => {
    ...
    if (requestUrl.pathname.endsWith('/projects/catalog')) { intercepted.catalog += 1; return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(envelope(catalog)) }); }
    if (requestUrl.pathname.endsWith('/projects/snapshot')) { intercepted.snapshot += 1; return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(envelope(snapshot)) }); }
  ...
  page.on('response', async (response) => {
    ...
    if (response.status() !== 200) { commitUnintercepted += 1; return; }
    try {
      const body = await response.json();
      const data = body?.data ?? {};
      const validShape = body?.ok === true && typeof data.hash === 'string' && /^[0-9a-fA-F]{40}$/.test(data.hash) && typeof data.subject === 'string' && Array.isArray(data.files) && typeof data.diff === 'string';
  ...
  const assertColumnOwnership = (state) => {
    ...
    if (sidebar) {
      ...
      if (width > 480) {
        check(`${state} sidebar is an independent vertical scroll owner`, ['auto', 'scroll'].includes(sidebar.overflowY), `overflowY ${sidebar.overflowY}`);
        check(`${state} sidebar is bounded to the viewport`, sidebar.rect.height <= height, `height ${sidebar.rect.height} viewport ${height}`);
      } else {
        // Mobile: the route is the single page scroll owner; the sidebar must not become a second one.
        check(`${state} mobile sidebar is not a nested scroll owner`, sidebar.scrollHeight <= sidebar.clientHeight + 1, `${sidebar.scrollHeight}/${sidebar.clientHeight}`);
      }
    }
    if (column) check(`${state} context column is bounded`, column.overflowX === 'hidden' && column.overflowY === 'hidden', `${column.overflowX}/${column.overflowY}`);
  };
  ...
  await expectCounts(page, 'diff', { 'context-section': 1, 'context-body': 1, 'context-breadcrumb': 1, 'context-breadcrumb-branch': 0, 'context-diff': 1, 'git-log-terminal': 0, 'context-commit-detail': 0, 'git-log-breadcrumb': 0 });
  geometry.diff = { shell: await measure(page, 'context-section'), breadcrumb: await measure(page, 'context-breadcrumb'), body: await measure(page, 'context-body') };
  ...
  if (width > 480) {
    ...
    check('diff shell bounded to viewport', (geometry.diff.shell?.rect.height ?? 0) <= height, `height ${geometry.diff.shell?.rect.height} viewport ${height}`);
    check('diff shell not scrolling', geometry.diff.shell?.scrollHeight === geometry.diff.shell?.clientHeight, `${geometry.diff.shell?.scrollHeight}/${geometry.diff.shell?.clientHeight}`);
    await assertScrollable(page, 'diff', 'context-diff', { horizontal: true });
  } else {
    await assertMobileNonOwner(page, 'mobile-diff', 'context-diff');
  }
  check('diff shows no duplicate header or back control', (await page.locator('text="Diff del file selezionato"').count()) === 0 && (await page.locator('text="Branch log ·"').count()) === 0 && (await page.locator('button[aria-label="Back to branch log"]').count()) === 0);
  ...
  await expectCounts(page, 'log', { 'context-section': 1, 'context-body': 1, 'context-breadcrumb': 1, 'context-breadcrumb-branch': 0, 'context-diff': 0, 'git-log-terminal': 1, 'git-log-commit-row': LONG_COMMITS, 'git-log-ref': LONG_REFS.length, 'context-commit-detail': 0, 'context-commit-scroll': 0 });
  geometry.log = { shell: await measure(page, 'context-section'), breadcrumb: await measure(page, 'context-breadcrumb'), body: await measure(page, 'context-body') };
  ...
  if (width > 480) {
    ...
    check('log shell same height as diff', Math.abs((geometry.log.shell?.rect.height ?? 0) - (geometry.diff.shell?.rect.height ?? 0)) <= GEOMETRY_TOLERANCE);
    check('log shell not scrolling', geometry.log.shell?.scrollTop === 0 && geometry.log.shell?.scrollHeight === geometry.log.shell?.clientHeight);
    check('log body not scrolling', geometry.log.body?.scrollTop === 0 && geometry.log.body?.scrollHeight === geometry.log.body?.clientHeight);
    ...
    check('log shell not scrolling on mobile', geometry.log.shell?.scrollHeight === geometry.log.shell?.clientHeight, `${geometry.log.shell?.scrollHeight}/${geometry.log.shell?.clientHeight}`);
  ...
  await expectCounts(page, 'commit', { 'context-section': 1, 'context-body': 1, 'context-breadcrumb': 1, 'context-breadcrumb-branch': 1, 'context-commit-detail': 1, 'context-commit-scroll': 1, 'git-log-terminal': 0, 'git-log-commit-row': 0, 'context-diff': 0 });
  check('commit did not intercept /projects/commit', commitUnintercepted === 0, `bad responses ${commitUnintercepted}`);
  geometry.commit = { shell: await measure(page, 'context-section'), breadcrumb: await measure(page, 'context-breadcrumb'), body: await measure(page, 'context-body') };
  ...
  if (width > 480) {
    check('commit shell same height as diff', Math.abs((geometry.commit.shell?.rect.height ?? 0) - (geometry.diff.shell?.rect.height ?? 0)) <= GEOMETRY_TOLERANCE);
    check('commit shell not scrolling', geometry.commit.shell?.scrollTop === 0 && geometry.commit.shell?.scrollHeight === geometry.commit.shell?.clientHeight);
    await assertScrollable(page, 'commit', 'context-commit-scroll', { horizontal: false });
  } else {
    check('commit shell not scrolling on mobile', geometry.commit.shell?.scrollHeight === geometry.commit.shell?.clientHeight, `${geometry.commit.shell?.scrollHeight}/${geometry.commit.shell?.clientHeight}`);
  ...
  await page.waitForSelector('[data-testid="context-commit-detail"]', { timeout: INTERACTION_TIMEOUT_MS });
  await expectCounts(page, 'timeline-commit', { 'context-commit-detail': 1, 'context-breadcrumb-branch': 0, 'git-log-terminal': 0 });
  check('timeline commit calls /projects/commit', commitRequests.length > commitBefore, `before ${commitBefore} after ${commitRequests.length}`);
  const commitQuery = commitRequests.at(-1)?.query ?? {};
  check('commit request carries required query', ['project_id', 'commit', 'local_generation', 'processInstanceId', 'registryEpoch', 'contextIdentity', 'snapshotId'].every((key) => commitQuery[key] !== undefined), JSON.stringify(commitQuery));
  ...
  if (width > 480) {
    ...
    check('mobile-after-reload route owns vertical scroll', mobileRoute.overflowY === 'auto' || mobileRoute.overflowY === 'scroll', mobileRoute.overflowY);
    check('mobile-after-reload route has scrollable content', mobileRoute.scrollHeight > mobileRoute.clientHeight, `${mobileRoute.scrollHeight}/${mobileRoute.clientHeight}`);
    await page.locator('[data-tree-path="src/app.ts"]').first().click();
    await page.waitForSelector('[data-testid="context-diff"]', { timeout: INTERACTION_TIMEOUT_MS });
    await assertMobileNonOwner(page, 'mobile-diff-after-reload', 'context-diff');
    const nestedAfterReload = await page.evaluate(() => Array.from(document.querySelectorAll('[data-testid="mc-projects-route"] *')).filter((element) => {
      const style = getComputedStyle(element);
      const id = element.getAttribute('data-testid');
      return /(auto|scroll)/.test(style.overflowY) && !['context-diff', 'git-log-scroll', 'context-commit-scroll'].includes(id) && !element.closest('#mc-project-sidebar');
    }).filter((element) => element.getBoundingClientRect().height >= window.innerHeight - 1).map((element) => element.getAttribute('data-testid')));
    check('mobile-after-reload no nested full-height scroll owner', nestedAfterReload.length === 0, JSON.stringify(nestedAfterReload));
  }

  // --- scroll ownership of the whole route (no competing nested owner)
  const nested = await page.evaluate(() => Array.from(document.querySelectorAll('[data-testid="mc-projects-route"] *')).filter((element) => {
    const style = getComputedStyle(element);
    const scrollable = /(auto|scroll)/.test(style.overflowY);
    const id = element.getAttribute('data-testid');
    return scrollable && !['context-diff', 'git-log-scroll', 'context-commit-scroll'].includes(id) && !element.closest('#mc-project-sidebar');
  }).map((element) => ({ testId: element.getAttribute('data-testid'), id: element.id, overflowY: getComputedStyle(element).overflowY, height: element.getBoundingClientRect().height })));
  ...
  metrics.route = route;
  if (width <= 480) {
    check('mobile route owns vertical scroll', route.overflowY === 'auto' || route.overflowY === 'scroll', route.overflowY);
    check('mobile nested full-height owner absent', nested.every((entry) => entry.height < height), JSON.stringify(nested));
  } else {
  ...
  const pluginErrors = consoleErrors.filter((message) => !unattributable.test(message) && !hostShellErrors.test(message));
  ...
  check('plugin catalog/snapshot/commit responses all 200', ['catalog', 'snapshot', 'commit'].every((name) => pluginEndpointResponses.some((entry) => entry.endpoint === name && entry.status === 200)), JSON.stringify(pluginEndpointResponses));
  check('commit endpoint served by the fixture dispatcher', commitUnintercepted === 0 && commitRequests.length > 0, `requests ${commitRequests.length} invalid ${commitUnintercepted}`);
  check('no plugin console/page errors', pluginErrors.length === 0, pluginErrors.slice(0, 3).join(' | '));
  const pluginFailures = await page.evaluate(() => Array.from(document.querySelectorAll('[data-testid="mc-projects-route"] [role="status"]')).map((element) => element.textContent).filter((text) => /unavailable|error|failed/i.test(text ?? '')));
  ...
  process.stdout.write(`${browserKey} ${viewportArg}: ${metrics.checks.length - failures.length}/${metrics.checks.length} checks passed; artifacts ${outDir}\n`);
  if (failures.length) { process.stderr.write(`FAIL\n- ${failures.join('\n- ')}\n`); process.exit(1); }
  process.stdout.write('PASS\n');
}

main().catch((error) => { process.stderr.write(`issue1-browser-verification: ${error?.stack ?? error}\n`); process.exit(3); });
