...
export interface ProjectBranch {
  ...
  repository?: string | null
  relation?: 'up-to-date' | 'ahead' | 'behind' | 'diverged' | 'no-upstream'
  ahead?: number
  behind?: number
}
...
export interface Snapshot {
  ...
  project_id: string
  project: { name: string; repository?: string | null }
  snapshotId: string
  head?: string | null
  observedAt: string
  refreshing: boolean
  ...
  branches: { local: ProjectBranch[]; remote: ProjectBranch[]; remoteAlias: string; repository?: string | null }
  commits: Commit[]
  fileDiffs: Record<string, string>
  branchLogs: Record<string, Commit[]>
  focusDefaults?: { kind: FocusKind; value?: string | null }
  github: { status: string; pullRequests: PullRequest[]; issues: Issue[] }
  warnings?: string[]
  localStatus?: string
