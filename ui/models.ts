...
export function safeHttpsUrl(value: string | undefined, expectation?: GitHubUrlExpectation): string | undefined {
  ...
  if (/%2e|%2f|%5c/i.test(value) || value.includes('\\') || /\/(?:\.{1,2})(?:\/|$)/.test(value)) return undefined;
  try {
...
export function focusFromSnapshot(snapshot: Pick<Snapshot, 'focusDefaults' | 'workingTree' | 'branches' | 'commits' | 'fileDiffs' | 'branchLogs'>): Focus | null {
  const preferred = snapshot.focusDefaults;
  if (preferred?.value) {
    const valid = preferred.kind === 'file' && (snapshot.workingTree.files.some((file) => file.path === preferred.value) || preferred.value in snapshot.fileDiffs)
      || preferred.kind === 'branch' && (snapshot.branches.local.some((branch) => branch.name === preferred.value) || snapshot.branches.remote.some((branch) => branch.name === preferred.value) || preferred.value in snapshot.branchLogs)
      || preferred.kind === 'commit' && (snapshot.commits.some((commit) => commit.hash === preferred.value) || Object.values(snapshot.branchLogs).some((entries) => entries.some((commit) => commit.hash === preferred.value)));
    if (valid) return { kind: preferred.kind, value: preferred.value };
  }
  const file = snapshot.workingTree.files[0]?.path;
  if (file) return { kind: 'file', value: file };
  const branch = snapshot.branches.local[0]?.name ?? snapshot.branches.remote[0]?.name ?? Object.keys(snapshot.branchLogs)[0];
