...
export function flattenFiles(tree: { files?: FileEntry[] }): FileRow[] {
  ...
  for (const file of tree.files ?? []) {
    ...
    for (let i = 1; i < parts.length; i += 1) {
      ...
      if (!folders.has(path)) {
        folders.add(path)
        rows.push({ path, label: parts[i - 1], depth: i - 1, kind: 'folder' })
      }
    }
    rows.push({
      path: file.path,
...
export function canMutate(snapshot: Pick<Snapshot, 'workingTree' | 'capabilities' | 'refreshing'>): boolean {
  ...
  return Object.values(snapshot.capabilities).every(
    (cap) => !cap.stale && cap.status !== 'stale' && cap.status !== 'unavailable' && cap.status !== 'error',
  )
}

export function focusFromSnapshot(
  snapshot: Pick<Snapshot, 'focusDefaults' | 'workingTree' | 'branches' | 'commits' | 'fileDiffs' | 'branchLogs'>,
): Focus | null {
  const preferred = snapshot.focusDefaults
  if (preferred?.value) {
    const valid =
      (preferred.kind === 'file' &&
        (snapshot.workingTree.files.some((file) => file.path === preferred.value) ||
          preferred.value in snapshot.fileDiffs)) ||
      (preferred.kind === 'branch' &&
        (snapshot.branches.local.some((branch) => branch.name === preferred.value) ||
          snapshot.branches.remote.some((branch) => branch.name === preferred.value) ||
          preferred.value in snapshot.branchLogs)) ||
      (preferred.kind === 'commit' &&
        (snapshot.commits.some((commit) => commit.hash === preferred.value) ||
          Object.values(snapshot.branchLogs).some((entries) =>
            entries.some((commit) => commit.hash === preferred.value),
          )))
  ...
  const branch =
    snapshot.branches.local[0]?.name ?? snapshot.branches.remote[0]?.name ?? Object.keys(snapshot.branchLogs)[0]
