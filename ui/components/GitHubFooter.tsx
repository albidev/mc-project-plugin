 ...
 */
export function GitHubFooter({ issues, pullRequests, status, bare }: { issues: Issue[]; pullRequests: PullRequest[]; status: string; bare?: boolean }) {
  if (!issues.length && !pullRequests.length) return null;
  const wrap = bare ? 'min-w-0 py-0.5' : 'grid min-w-0 gap-2';
  const rowCls = bare ? 'gh-item-row' : 'flex min-h-11 items-center gap-3 border-b border-border px-3 text-xs hover:bg-surface-sunken/40';
  const issueRow = (issue: Issue) => {
    ...
    return href
      ? <a key={issue.number} href={href} target="_blank" rel="noreferrer" title={issue.title} className={rowCls}>{inner}</a>
      : <div key={issue.number} title={issue.title} className={rowCls}>{inner}</div>;
  };
  const prRow = (pr: PullRequest) => {
    ...
    return href
      ? <a key={pr.number} href={href} target="_blank" rel="noreferrer" aria-label={`Open pull request #${pr.number}`} title={pr.title} className={rowCls}>{inner}</a>
      : <div key={pr.number} title={pr.title} className={rowCls}>{inner}</div>;
  };
