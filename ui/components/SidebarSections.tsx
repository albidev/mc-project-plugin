...
function AccSection({ id, title, count, open, onToggle, children, trailing }: { id: string; title: string; count?: number; open: boolean; onToggle: () => void; children: React.ReactNode; trailing?: React.ReactNode }) {
  return <div className={`acc-section ${open ? 'open' : ''}`} data-acc-section={id}>
    ...
    <div id={`${id}-body`} className="acc-body"><div className="acc-body-inner">{children}</div></div>
  </div>;
}
 ...
 */
export function SidebarSections({ snapshot, focus, onSelectFile, onSelectBranch, onSelectCommit, onSwitch, onCreate, mutationMessage, mutationBusy, selectedCommitHash }: { snapshot: Snapshot; focus: Focus | null; onSelectFile: (p: string) => void; onSelectBranch: (n: string) => void; onSelectCommit: (h: string) => void; onSwitch?: (n: string) => void; onCreate?: (n: string) => void; mutationMessage?: string; mutationBusy?: boolean; selectedCommitHash?: string }) {
    ...
    <AccSection id="branches" title="Branch topology" open={open.branches} onToggle={() => toggle('branches')}>
      <div className="branch-iconbar" role="group" aria-label="Branch view and actions">
        <span className="branch-icon-group">
          <button type="button" className={`branch-icon-tab ${branchTab === 'local' ? 'active' : ''}`} aria-pressed={branchTab === 'local'} aria-label={`Local branches (${localCount})`} title={`Local branches (${localCount})`} onClick={() => setBranchTab('local')}><GitBranch size={13} /><span className="branch-icon-count">{localCount}</span></button>
          <button type="button" className={`branch-icon-tab ${branchTab === 'remote' ? 'active' : ''}`} aria-pressed={branchTab === 'remote'} aria-label={`Remote branches (${remoteCount})`} title={`Remote branches (${remoteCount})`} onClick={() => setBranchTab('remote')}><GitFork size={13} /><span className="branch-icon-count">{remoteCount}</span></button>
        </span>
        {currentBranch && onCreate && <button type="button" className="branch-icon-tab branch-icon-action" aria-label={`Create branch from ${currentBranch}`} title={`Create a branch from ${currentBranch}`} onClick={() => setCreateOpen((v) => !v)}><Plus size={13} /></button>}
      </div>
