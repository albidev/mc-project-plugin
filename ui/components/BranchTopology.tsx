 ...
 */
export function BranchTopology({ local, remote, onSelect, onSwitch, onCreate, mutationMessage, mutationBusy, tab, onTabChange, createOpen, onCreateOpenChange }: { local: ProjectBranch[]; remote: ProjectBranch[]; onSelect: (name: string) => void; onSwitch?: (name: string) => void; onCreate?: (name: string) => void; mutationMessage?: string; mutationBusy?: boolean; tab?: 'local' | 'remote'; onTabChange?: (t: 'local' | 'remote') => void; createOpen?: boolean; onCreateOpenChange?: (open: boolean) => void }) {
  ...
  return <div data-testid="branches-section" className="branch-topology min-w-0">
    {activeTab === 'local' && current && isCreateOpen && <div className="branch-create-modal"><input aria-label="Branch name" value={createName} onChange={(e) => setCreateName(e.currentTarget.value)} onInput={(e) => setCreateName(e.currentTarget.value)} onKeyDown={(e) => { if (e.key === 'Enter') submitCreate(); if (e.key === 'Escape') setCreateOpen(false); }} autoFocus className="branch-create-input" placeholder="feature/name" /><button type="button" disabled={!createName.trim()} onClick={submitCreate} className="branch-create-confirm">Create</button></div>}
    <div className="branch-list">{items.map((branch) => <div key={branch.name} data-branch-name={branch.name} className={`branch-row ${branch.current ? 'current' : ''}`} onClick={() => onSelect(branch.name)} onDoubleClick={() => { if (activeTab === 'local' && onSwitch && branch.current !== true) onSwitch(branch.name); }}>
      <span className={`bcurrent-dot ${branch.current ? 'is-current' : ''}`} title={branch.current ? 'Current branch' : 'Not checked out'} aria-hidden="true" />
      <span className="branch-main">
        <span className="bname" title={branch.name}>{branch.name}</span>
        <span className={`bstate state-${branchStateKey(branch)}`}>{branchStateLabel(branch)}</span>
      </span>
      {activeTab === 'local' && onSwitch && branch.current !== true && <button type="button" title="Switch to this branch" aria-label={`Switch to ${branch.name}`} className="bswitch" onClick={(e) => { e.stopPropagation(); onSwitch(branch.name); }}><ArrowRightLeft size={13} /></button>}
    </div>)}</div>
