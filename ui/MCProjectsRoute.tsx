...
export default function MCProjectsRoute() {
  ...
  return <main data-testid="mc-projects-route" className="flex h-full max-h-full min-h-0 w-full max-w-full min-w-0 flex-col overflow-x-hidden overflow-y-auto overscroll-contain bg-surface text-text md:overflow-hidden"><header id="mc-project-header" className="flex min-h-9 shrink-0 items-center gap-2 px-4 py-1 sm:px-5"><ProjectSelector active={active} projects={selectorOptions(catalog, active.project_id)} open={selectorOpen} onToggle={controller.toggleSelector} onSelect={selectProject} activeBranch={snapshot.branches.local.find((b) => b.current)?.name} activeChanged={snapshot.workingTree.files?.length} /></header>{loading && <div className="shrink-0 border-b border-border px-4 py-1 cp-11 text-text-muted" role="status">Refreshing; last-known-good data is shown.</div>}{error && <div className="shrink-0 border-b border-border px-4 py-1 cp-11 text-warning" role="status">Refresh unavailable; showing last-known-good data.</div>}<div className="grid min-h-0 flex-1 md:overflow-hidden md:grid-cols-[minmax(248px,20%)_minmax(0,1fr)]"><aside id="mc-project-sidebar" className="min-w-0 space-y-2 border-b p-2 md:min-h-0 md:overflow-y-auto md:border-b-0"><CapabilityNotice name="working tree" capability={snapshot.capabilities.workingTree as Snapshot['capabilities'][string]} /><CapabilityNotice name="branches" capability={snapshot.capabilities.branches as Snapshot['capabilities'][string]} /><CapabilityNotice name="commits" capability={snapshot.capabilities.commits as Snapshot['capabilities'][string]} /><CapabilityNotice name="branch logs" capability={snapshot.capabilities.branchLogs as Snapshot['capabilities'][string]} /><CapabilityNotice name="GitHub" capability={snapshot.capabilities.github as Snapshot['capabilities'][string]} /><SidebarSections snapshot={snapshot} focus={focus} mutationMessage={mutationMessage} mutationBusy={mutationBusy} selectedCommitHash={selectedCommitHash} onSelectFile={(path) => void selectFocus({ kind: 'file', value: path })} onSelectBranch={(name) => void selectFocus({ kind: 'branch', value: name })} onSelectCommit={(hash) => void selectFocus({ kind: 'commit', value: hash })} onSwitch={handleBranchSwitch} onCreate={handleBranchCreate} /></aside><div id="mc-project-context-column" className="min-w-0 min-h-0 overflow-hidden md:h-full"><ContextPanel focus={focus} snapshot={snapshot} detail={detail} loading={detailLoading} selectedCommitHash={selectedCommitHash} onSelectCommit={(hash) => void controller.selectCommit(hash)} lastUpdated={snapshot.observedAt} onRefresh={() => void controller.refresh()} /></div></div></main>;
}
