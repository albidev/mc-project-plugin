...
const cap = (value, source = 'local_git') => ({ status: 'ready', stale: false, source, observedAt: '2026-09-14T00:00:00Z', generation: 1, value });
const commitValue = (index: number) => { const hash = String(index + 1).padStart(40, '0'); return { hash, shortHash: hash.slice(0, 7), subject: `Commit ${index}`, author: 'Test', date: '2026-09-14T00:00:00Z', merge: false, refs: [], parents: [] }; };
const snapshot = () => ({ schemaVersion: 1, project_id: 'demo', project: { name: 'Demo', repository: 'org/demo' }, snapshotId: 'snap-1', head: 'a'.repeat(40), observedAt: '2026-09-14T00:00:00Z', lastUpdated: '2026-09-14T00:00:00Z', processInstanceId: 'process-1', registryEpoch: 1, localGeneration: 1, contextIdentity: 'context-1', localStatus: 'ready', warnings: [], refreshing: false, fingerprints: { HEAD: 'a', status: 'b', 'refs/remotes': 'c', 'refs/heads': 'd', currentBranch: 'e', workingTree: 'f', branches: 'g', commits: 'h', local: 'i' }, workingTree: { files: [{ path: 'src/app.ts', status: 'M' }] }, fileDiffs: { 'src/app.ts': 'diff --git a/src/app.ts b/src/app.ts\\n+change' }, branchLogs: { main: [{ hash: 'a'.repeat(40), shortHash: 'aaaaaaa', subject: 'Initial', author: 'Test', date: '2026-09-14T00:00:00Z', merge: false, refs: [], parents: [] }] }, branches: { local: [{ name: 'main', current: true }], remote: [{ name: 'origin/main' }], remoteAlias: 'origin', repository: 'https://github.com/org/demo.git' }, commits: [{ hash: 'a'.repeat(40), shortHash: 'aaaaaaa', subject: 'Initial', author: 'Test', date: '2026-09-14T00:00:00Z', merge: false, refs: [], parents: [] }], github: { status: 'ready', pullRequests: [{ number: 1, title: 'Fix', url: 'https://github.com/org/demo/pull/1', repository: 'org/demo', head_repository: 'org/demo', base_repository: 'org/demo', created_at: '2026-09-14T00:00:00Z' }], issues: [] }, capabilities: { workingTree: cap({ files: [{ path: 'src/app.ts', status: 'M' }]}), branches: cap({ local: [{ name: 'main', current: true }], remote: [{ name: 'origin/main' }], remoteAlias: 'origin', repository: 'https://github.com/org/demo.git' }), commits: cap([{ hash: 'a'.repeat(40), shortHash: 'aaaaaaa', subject: 'Initial', author: 'Test', date: '2026-09-14T00:00:00Z', merge: false, refs: [], parents: [] }]), branchLogs: cap({ main: [{ hash: 'a'.repeat(40), shortHash: 'aaaaaaa', subject: 'Initial', author: 'Test', date: '2026-09-14T00:00:00Z', merge: false, refs: [], parents: [] }] }), github: cap({ status: 'ready', pullRequests: [{ number: 1, title: 'Fix', url: 'https://github.com/org/demo/pull/1', repository: 'org/demo', head_repository: 'org/demo', base_repository: 'org/demo', created_at: '2026-09-14T00:00:00Z' }], issues: [] }, 'github') }, focusDefaults: { kind: 'file', value: 'src/app.ts' } });
...
test('nested capability payloads and focus defaults are validated by key', () => {
  ...
  const divergent = snapshot(); divergent.capabilities.branchLogs.value = {};
  assert.equal(validSnapshot(divergent, 'demo'), false);
  const invalidStale = snapshot(); invalidStale.capabilities.commits.stale = true;
  assert.equal(validSnapshot(invalidStale, 'demo'), false);
  const missingObservedAt = snapshot(); delete missingObservedAt.capabilities.commits.observedAt;
...
test('all contract fixtures satisfy the frontend snapshot validator', () => {
  for (const name of ['snapshot-ready.json', 'snapshot-partial.json']) {
    const envelope = JSON.parse(readFileSync(fileURLToPath(new URL(`../../tests/fixtures/contracts/${name}`, import.meta.url)), 'utf8'));
...
test('accepts exact branch and branch-log limits and rejects the next item', () => {
  ...
  tooManyCommits.branchLogs.main = [...tooManyCommits.branchLogs.main, { hash: 'f'.repeat(40), subject: 'overflow', parents: [] }];
...
test('the route controller requires exact mutation acknowledgement and read-back before replacing state', async () => {
  ...
  assert.equal(reads, 2); assert.equal(badAck.getState().snapshot?.snapshotId, 'snap-2'); assert.match(badAck.getState().mutationMessage, /indeterminate/i);
  let readBackReads = 0;
...
test('validation errors do not trigger mutation recovery', async () => {
  ...
  let reads = 0;
  const rejected = createRouteController(routeApi({ demo: original }, {
    snapshot: async () => { reads += 1; return original; },
    switchBranch: async () => { throw new ApiError('INVALID_BRANCH_NAME', 'Invalid branch name.', 400); },
  }));
...
test('server indeterminate mutation errors still trigger recovery', async () => {
  const original = snapshot(); const recovered = { ...original, snapshotId: 'snap-recovered' };
  let reads = 0;
  const indeterminate = createRouteController(routeApi({ demo: original }, {
    snapshot: async () => { reads += 1; return reads === 1 ? original : recovered; },
    switchBranch: async () => { throw new ApiError('READBACK_MISMATCH', 'Mutation read-back did not confirm the requested branch.', 409); },
  }));
