...
test('renders root and linear history with one commit lane', () => {
  ...
  assert.deepEqual(rows.map((row) => row.hash), ['c', 'b', 'a']);
  assert.deepEqual(rows.map((row) => row.lanes), [1, 1, 1]);
...
test('renders every connector for a three-parent merge', () => {
  ...
  assert.deepEqual(rows[0].transitions.filter((transition) => transition.from !== transition.to).map((transition) => [transition.from, transition.to]), [[0, 1], [0, 2]]);
...
test('preserves refs and marks the selected commit', () => {
  const rows = buildGitGraph([
    commit('head', [], { refs: ['HEAD -> main', 'origin/main', 'tag: v1.0'] }),
  ], { selectedHash: 'head' });
