...
function graphLine(before: string[], position: number, transitions: GitGraphTransition[]): string {
  const width = Math.max(before.length, position + 1, ...transitions.map((transition) => transition.from + 1), ...transitions.map((transition) => transition.to + 1));
...
function advanceLanes(before: string[], position: number, parents: string[]): { lanes: string[]; transitions: GitGraphTransition[]; convergence: boolean } {
